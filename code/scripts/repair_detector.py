...
def detect_repairs(dialogue_data: Dict[str, Any], model=None, max_retries: int = 3) -> List[Dict[str, Any]]:
    """
    ...
    Returns:
        List of repair annotation dictionaries (validated and filtered)
    """
    if model is None:
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Validate and filter repairs before returning
            valid_repairs = []
            for repair in repairs:
                ...
                if is_valid:
                    ...
                    print(f"  [WARNING] Invalid repair detected and filtered out:")
                    for warning in warnings:
                        print(f"    - {warning}")
                    print(f"    Repair data: {repair.get('repair_id', 'unknown')}, turns: {repair.get('turn_indices', [])}")
            
            if len(valid_repairs) < len(repairs):
                print(f"  [INFO] Filtered {len(repairs) - len(valid_repairs)} invalid repair(s) out of {len(repairs)} total")
            
            return valid_repairs
            
        except Exception as e:
            if attempt < max_retries - 1:
                print(f"  [RETRY] Error calling Gemini API (attempt {attempt + 1}/{max_retries}): {e}")
                import time
                time.sleep(2 ** attempt)  # Exponential backoff
            else:
                print(f"  [ERROR] Failed to call Gemini API after {max_retries} attempts: {e}")
                return []
...
def validate_repair_annotation(repair: Dict[str, Any], dialogue_id: str, dialogue_data: Optional[Dict[str, Any]] = None) -> tuple:
    ...
    Args:
        repair: Repair annotation dictionary
        dialogue_id: Expected dialogue ID
        dialogue_data: Optional dialogue data for turn index validation
    ...
    # Validate turn_indices are valid integers
    ...
    # Validate turn indices are within bounds (if dialogue_data provided)
    if dialogue_data is not None:
        ...
        # Check if turn indices are in order
        if repair['turn_indices'] != sorted(repair['turn_indices']):
            ...
            # Don't fail for this, just warn - we can fix it
